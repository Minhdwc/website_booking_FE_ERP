...
export const VenueImagesSection = ({ venueId, images }: VenueImagesSectionProps) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const [isDragging, setIsDragging] = useState(false);

  const uploadMutation = useUploadVenueImage();
  const deleteMutation = useDeleteVenueImage();
  const thumbnailMutation = useSetVenueImageThumbnail();
  ...
  const uploadFiles = async (fileList: FileList | null) => {
    ...
    for (const file of files) {
      try {
        await uploadMutation.mutateAsync({ venueId, file });
        uploaded += 1;
      } catch (error: unknown) {
  ...
  const handleDelete = async (image: IVenueImage) => {
    if (!window.confirm('Xóa ảnh này khỏi cơ sở?')) return;
    try {
      await deleteMutation.mutateAsync({ venueId, imageId: image.id });
  ...
  const handleSetThumbnail = async (image: IVenueImage) => {
    if (image.isThumbnail) return;
    try {
      await thumbnailMutation.mutateAsync({ venueId, imageId: image.id });
